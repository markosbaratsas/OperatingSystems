...
struct kgarten_struct {
	...
	sem_t sem_teachers, sem_children;
...
void child_enter(struct thread_info_struct *thr)
{
	...
	while(1) {
		...
		int this_val;
		sem_getvalue(&thr->kg->sem_children,&this_val);
		if ((this_val+1) <= (thr->kg->ratio) * (thr->kg->vt)) {
			sem_post(&thr->kg->sem_children);
			++(thr->kg->vc);
...
void teacher_exit(struct thread_info_struct *thr)
{
	...
	while(1) {
		pthread_mutex_lock(&thr->kg->mutex);
		int this_val;
		sem_getvalue(&thr->kg->sem_teachers,&this_val);
		if ((thr->kg->vc) <= (thr->kg->ratio) * (this_val-1)) {
			sem_wait(&thr->kg->sem_teachers);
			--(thr->kg->vt);
			pthread_mutex_unlock(&thr->kg->mutex);
			break;
		}
...
int main(int argc, char *argv[])
{
	...
	sem_init(&kg->sem_teachers, 0, 0);
	sem_init(&kg->sem_children, 0, 0);
	for (i = 0; i < thrcnt; i++) {
